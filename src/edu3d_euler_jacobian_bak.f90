...
!--------------------------------------------------------------------------------
! Close the Jacobian by the derivative of the general boundary flux quadrature.
!
! Loop over boundary elements and distribute a contribution to each node. 
! Node ordering (orientation) is not important here.
! For each node, j, we call the other nodes, l and r.
!
!     l              r
!      o------------o
!       \         .
...
!
!
! - General boundary flux quadrature formula is described in:
!
! H. Nishikawa, " Accuracy-preserving boundary flux quadrature for finite-volume
! discretization on unstructured grids", Journal of Computational Physics,
! Volume 281, January 2015, Pages 518-555, 2015.
! https://doi.org/10.1016/j.jcp.2014.10.033
! http://ossanworld.com/hiroakinishikawa/My_papers/nishikawa_jcp2015v281pp518-555_preprint.pdf
!
!
! - Second-order quadrature formulas for 3D tet/prs/pyr/hex grids can be found in:
!
...
!--------------------------------------------------------------------------------
  ...
  boundary_element_loop : do i = 1, ntria
    ...
    nodes_of_triangle : do j = 1, 3 ! Nodes: nj, nr, nl

    !----------------------------------------------------------------
    ! Node j: Phi_jb(bn): BC is implemented here.

     nj = tria( i, j )
     call get_right_state(  bc_type(ib),w(nj,:),bnj(i,j,:),x(nj),y(nj),z(nj),  duRduL,wb )
     call interface_jac_b( w(nj,:), wb, bn(i,:), dFnduL, dFnduR )

     !Contribution to node j (diagonal block):
      jac_diag(nj,:,:) = jac_diag(nj,:,:) + half*(dFnduL + matmul(dFnduR,duRduL)) * (third*bn_mag(i))

    !----------------------------------------------------------------
    ! Flux between j and r: Phi_jr(bn): No boundary condition.

          nr = tria( i, loc_nghbr(j,1) )

     call interface_jac( w(nj,:), w(nr,:), bn(i,:), dFnduL, dFnduR )

     !Contribution to node j (diagonal block):
      jac_diag(nj,:,:) = jac_diag(nj,:,:) + fourth*(dFnduL) * (third*bn_mag(i))

     !Contribution to node j (off-diagonal block):
      k = kth_nghbr(nj,nr)
      jac_off( k0(nj)+k ,:,:) = jac_off( k0(nj)+k, :,:) + fourth*(dFnduR) * (third*bn_mag(i))

    !----------------------------------------------------------------
    ! Flux between j and l: Phi_jl(bn): No boundary condition.

          nl = tria( i, loc_nghbr(j,2) )

     call interface_jac(  w(nj,:), w(nl,:), bn(i,:), dFnduL, dFnduR )
