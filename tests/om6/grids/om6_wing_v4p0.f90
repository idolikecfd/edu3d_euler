...
!
! Version 4.1 (Modified from Version 4.0, June 29, 2017).
! - Added command-line option to specify input file path
!
!------------------------------------------------------------------------------
!
!-------------
!  Input:  Parameters are set in the input file named as 'input.nml' by default,
!         or a custom file path provided as a command-line argument.
...
!       "Reynolds-averaged Navier-Stokes Simulations on NACA0012 and ONERA-M6
!        Wing with the ONERA elsA Solver" by Mayeur, A. Dumont, D. Destarac,
!        and V. Gleize. (TE closure is also as described in this paper.)

  character(256) :: airfoil_data_file = 'om6_wing_section_sharp.dat'
...
!*****************************************************************************
  ...
  if (os /= 0) then
    write(*,*) "ERROR: Could not open input file: ", namelist_file
    write(*,*) "Please check if the file exists and is readable."
    stop
  endif
...
! For command line argument processing
  ...
  character(len=256) :: namelist_file = 'input.nml'  ! Default input filename
...
do while (i_arg <= n_args)
  ...
  case ('-airfoil', '--airfoil-data-file')
    if (i_arg + 1 <= n_args) then
      i_arg = i_arg + 1
      call get_command_argument(i_arg, airfoil_data_file)
    else
      write(*,*) 'Error: Missing value after ', trim(arg)
      stop
    endif
...
if (show_help) then
  write(*,*) 'Usage: om6_wing [options]'
  write(*,*) 'Options:'
  write(*,*) '  -h, --help                              Show this help message'
  write(*,*) '  -nml, --namelist FILE                   Set input namelist file'
  write(*,*) '                                          (default: input.nml)'
  write(*,*) '  -airfoil, --airfoil-data-file FILE      Set input namelist file'
  write(*,*) '                                          (default: om6_wing_section_sharp.dat)'
  stop
endif
...
!*******************************************************************************
! Read the input parameters, defined in the file provided as a command line argument or default 'input.nml'.
!*******************************************************************************
